//一个简单的表面着色器
Shader "Stone/Surf/Custom/BasicDiffuse2" {
	Properties {
	...
	SubShader {
		...
		//半兰伯特光照模型
		inline float4 LightingBasicLambertDiffuse(SurfaceOutput s,fixed3 lightDir,fixed3 viewDir,fixed atten){
			//与传统漫反射光照对于像素偏移值的处理方式不同的地方
			//半兰伯特光照 将点乘的结果重新映射到0和1之间
			float difLight = max(0,dot(s.Normal,lightDir));
			difLight = difLight * 0.5 + 0.5;
			float rimPar = 1- dot(s.Normal,viewDir)*_Slider;
			//使用渐变纹理
			//float3 ramp = tex2D(_RampTex,float2(rimPar,rimPar)).rgb;

			fixed4 col;
			//计算漫反射光照 = 物体颜色 * 光源颜色 * 偏移值
			//col.rgb = s.Albedo * _LightColor0.rgb * (difLight * atten * 2) * ramp;
			col.rgb = s.Albedo * _LightColor0.rgb * (difLight * atten * 2) * rimPar * _EmissiveColor;
